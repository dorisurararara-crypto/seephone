...
### 2026-06-10 20:10 (Mac → Windows)

**batch_007 확정 ✅ — 왕복 루프 종료**

v2 검수 결과: crystal_v2_1(백수정 — 딱 원하던 것), guru_v2_1·v2_2(갈색/흑단 둘 다 채택 — 합성 잘됐다), knot_v2_1(진홍 단추매듭) 전원 통과. knot_v2_2(회색 혼합)만 미사용.
앱 통합 완료: 수정이 4번째 염주 재질로, 구루 알은 12시 실사 스프라이트(재질 따라 갈/흑 전환), 매듭은 완성 의례용으로 보관. 수고 많았다 — 이번 배치 품질 좋았음(특히 플랜 카드 4종).
다음 배치 발주 시 다시 HANDOFF로. 폴링 루프는 종료해도 된다.
...
- 용도: ①염주 화면에 '비즈가 실제로 모이는' 그래픽(투명배경 스프라이트 6종 — 보리수/흑단/수정/연자/구루비즈/매듭) ②수행 길(7일·30일·180일·365일) 카드 배경 4종 ③명상·공유 세로 배경 2종
- 스타일 고정: **감지금니(紺紙金泥)** — 남빛 종이(#0E1118 톤) + 가는 금니 선. negative에 kawaii/cartoon/cute 넣어둠(이 앱은 박신앱과 톤이 완전 다름 — 절제·고급).
- 투명배경 항목(transparent: true)은 rembg 등으로 배경 제거 후 PNG로.
- 결과는 `raw-images/batch_007/`에 항목 id 파일명으로. 후보 2~3장씩이면 Mac에서 큐레이션.

완료되면 이 블록 아래 결과 보고 부탁. (Mac은 2분 폴링 모드로 대기)
" 블록 확인
...
- **스타일 락(전 에셋 공통)**: 감지금니(deep indigo #0E1118 + 따뜻한 금니 #C9A24B~#E7C873 + 은니 silver #9AA3B8). 한국 사경(寫經)×수묵(sumi-e)×금박(geumbak) painterly. **형광/순금(#FFD700) 금지, 스톡 부처 정면사진 금지, 사진사실주의 금지.** 여백 넉넉(수묵 농담). SDXL이면 Sumi-e/Ink-wash LoRA 0.5~0.7 권장(있으면).
- 비율은 각 프롬프트 끝에 명시(9:16 세로배경, 1:1 정사각, 3:2 가로헤더, wide 디바이더). 텍스처류(goldleaf/indigo_paper/incense)는 가능하면 seamless.
- 12종: 메인 세로배경·홈 히어로(달+연꽃)·챕터 산수헤더·금박텍스처·인디고종이텍스처·연꽃선묘·명상만다라·달빛수면(슬립)·향연기·디바이더띠·앱아이콘모티프·빈상태(명상 실루엣).
- 후보 많이 뽑아줘(키당 3장이면 큐레이션 폭↑). Mac에서 2분 폴링 중 — 다 되면 push만 해줘.

다음 배치(batch_009) 예고: 10일 코스 경로 노드(완료/현재/잠김 3상태)·동자승 동반자 4단계·연꽃 개화 시퀀스 — 코스 화면 확정 후 발주 예정.

### 2026-06-10 21:35 (Windows → Mac)

**batch_007 v2 결과 — 재생성 3항목 push 완료, 검수 요청** 🔁

`raw-images/batch_007/`에 `_v2_N.png` 7장 추가 (전부 512px, 알파 테두리 검증 OK):

| 항목 | 후보 | 비고 |
|---|---|---|
| jeomju_bead_crystal_v2 | 3장 | Mac 수정 프롬프트 그대로 1발 통과 — 유백색 프로스트 단일 구슬, 풍경반사/금속느낌 제거됨 |
| jeomju_knot_v2 | 2장 | "button knot"을 SDXL이 몰라서 "woven silk ball button"으로 재표현 후 성공. _1=진홍 단색 둥근 단추매듭(추천), _2=적+회흑 투톤(단일이긴 함). 사탕/바퀴/덩굴로 나온 실패작은 제외 |
| jeomju_guru_bead_v2 | 2장 | ⚠️ **생성이 아니라 합성**: 수정 프롬프트 포함 5라운드 전부 말라 전체/추상 오브제로 붕괴 ("tassel"+"bead" 조합이 말라를 강하게 끌고 옴). 그래서 채택된 비즈 몸체(_1=보리수, _2=흑단) + 이전 생성물에서 추출한 진홍 태슬 + PIL로 그린 평평한 금캡(스레드홀 포함)으로 결정론 합성. 스크립트 `scripts/composite_guru_bead.py` 커밋됨 — 캡 크기/색/태슬 비율 등 수치 피드백 주면 분 단위로 재조정 가능 |

채택 9항목은 그대로 둠. guru 합성 캡이 톤에 안 맞으면 구체 수치(캡 폭 비율, 금색 톤, 태슬 길이)로 알려주면 바로 반영. (Windows 폴링 루프 계속 대기 중)
