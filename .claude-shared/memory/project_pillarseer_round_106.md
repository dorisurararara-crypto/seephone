...
- **배포 완료 — 1.0.0+68** (2026-05-21, 사용자 "출시" 후). commit `2400ad2`. +67(commit `2173ae7`) 먼저 ship 후, 보류했던 home_screen Oracle Hero 멘트 v5 재작성분을 +68 로 재배포 → +68 이 +67 대체. build 68 ASC VALID → 외부 그룹 `ganzitester` 할당(HTTP 204) → Beta Review 제출 완료. whatsNew ko/en 200. Public link `testflight.apple.com/join/kRs36R3b`. submit script = `scripts/submit_b68.rb`.
- **홈 '오늘의 한 줄' 미스터리형 전환** (사용자 mandate "미스터리형 친구형 중 미스터리형 골랐다"). `_OracleHero` 를 천간×dayEnergy 30 ment pool → **사주 미스터리형**으로 전환: 오늘 일진 지지 글자를 신비하게 던지고(예 `卯(묘)`), 사용자 일주와의 관계(충/합/friction/neutral, `TodayEventService.hapChungType` 계산값만)를 구조로 표현, "아래 풀이에" 호기심 유도. ment KO 20 + EN 20 (4관계×5변주). `MysteryRelation` 인프라(R106 P2b) 재활용. round71 test 미스터리형 재작성(12 test) + obsolete `round78_oracle_invariant_test`·`r86_oracle_hero_speech_test` 삭제(구 _pool 구조 제거).
- 최종 배포 **1.0.0+70 commit `df305ba`** — APPROVED, ganzitester 외부 베타 라이브. (+67~+69 는 ship 중간단계 / ASC 고아 build 69 회피로 +70 등록.) round82 version pin +70 동기화. `scripts/submit_b70.rb`.
...
## R106 미해결 / 차기
- codex 검수는 P4a/P4b/P5 모두 9.9 미달(goalpost 패턴) — hard violation 은 전수 수정, hedged 비교·구조형 read 는 scope reject. 사용자에게 A(현 상태 확정)/B(끝까지 추격) 선택 제시 → 실기기 판단 대기 중.
- 사용자가 "스타일 빼고 거짓말·명백단정·메타만" focused codex 재검수 요청 시 실행 대기.
- R106 자체는 P1~P6 + Oracle Hero v5 까지 전부 ship 완료. 차기 라운드는 사용자 실기기 1.0.0+68 피드백 후 결정.
