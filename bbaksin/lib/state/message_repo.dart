...
/// 시나리오: 특정 키워드 조합 → 전용 답변 풀.
/// match 의 각 그룹은 OR (그룹 안 단어 중 하나 이상 있어야), 모든 그룹은 AND (전체 그룹 모두 만족).
/// 빈 그룹은 자동 만족 (단일 키워드 시나리오).
class _Scenario {
  final String id;
  final List<List<String>> match;
  final List<String> answers;
  const _Scenario({required this.id, required this.match, required this.answers});

  bool matches(String lower) {
    for (final group in match) {
      if (group.isEmpty) continue;
      var groupHit = false;
      for (final kw in group) {
  ...
  /// 매칭 강도 (시나리오 간 우선순위) — 매칭된 키워드 수.
  int matchScore(String lower) {
    var s = 0;
    for (final group in match) {
      for (final kw in group) {
        if (lower.contains(kw.toLowerCase())) s++;
      }
    }
    return s;
  }
...
class MessageRepository {
  MessageRepository(this._messages, this._scenarios);
  ...
  Message pickFor(String question, {bool isRepeat = false}) {
    ...
    // === Phase 0: Scenario 매칭 (가장 정확) ===
    _Scenario? bestScenario;
    var bestScore = 0;
    for (final s in _scenarios) {
      if (!s.matches(lower)) continue;
      final score = s.matchScore(lower);
      if (score > bestScore) {
        bestScore = score;
        bestScenario = s;
      }
    ...
    if (bestScenario != null) {
      final ans = bestScenario.answers;
      return Message(ans[_rand.nextInt(ans.length)], 'scenario:${bestScenario.id}');
    }

    // === Phase 1: 카테고리 매칭 (fallback) ===
...
final messageRepoProvider = FutureProvider<MessageRepository>((ref) async {
  ...
  final messages = (json['messages'] as List)
      ...
      .toList();
  final scenariosJson = json['scenarios'] as List? ?? [];
  final scenarios = scenariosJson.map((e) {
    final m = e as Map<String, dynamic>;
    final matchRaw = m['match'] as List;
    final match = matchRaw
        .map((g) => (g as List).map((s) => s as String).toList())
        .toList();
    final answers = (m['answers'] as List).map((s) => s as String).toList();
    return _Scenario(id: m['id'] as String, match: match, answers: answers);
  }).toList();
  return MessageRepository(messages, scenarios);
});
