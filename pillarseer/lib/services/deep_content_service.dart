...
class DeepContentService {
  ...
  static const Map<String, String> _oneLineByJi60Ko = {
    // 甲(곧은 나무) ──
    '甲子': '어둠 속에서도 한 그루 나무처럼 천천히 키 키우는',
    '甲戌': '충직한 나무처럼 약속 끝까지 지키는',
    '甲申': '곧은 나무에 영리한 길 찾는 결 더한',
    '甲午': '곧은 나무에 한낮의 말 속도 더한',
    '甲辰': '큰 나무 옆 용 한 마리처럼 그릇이 큰',
    '甲寅': '곧은 나무에 범 한 마리처럼 단단한 의지 가진',
    // 乙(휘는 풀) ──
    '乙丑': '차가운 땅도 휘는 풀처럼 끈질기게 뚫고 올라오는',
    '乙亥': '따뜻한 풀처럼 사람 옆에 오래 머무는',
    '乙酉': '부드러운 줄기에 정확한 시간 감각을 가진',
    '乙未': '부드러운 풀처럼 따뜻한 마음 끝까지 가져가는',
    '乙巳': '부드러운 풀 안에 영리한 결 숨긴',
    '乙卯': '부드러운 봄 풀처럼 살랑살랑 사람을 끄는',
    // 丙(밝은 불) ──
    '丙寅': '새벽 햇살처럼 분위기 한 번에 환하게 바꾸는',
    '丙子': '한밤의 등불처럼 작아도 강하게 빛나는',
    '丙戌': '따뜻한 빛으로 옆 사람 지켜주는',
    '丙申': '환한 불에 영리함 더한',
    '丙午': '한낮의 불처럼 거침없이 빛나는',
    '丙辰': '환한 불 위 용처럼 큰 자리 만드는',
    // 丁(촛불) ──
    '丁卯': '촛불 한 자루로 사람 마음 끌어당기는',
    '丁丑': '어둠 속 모닥불처럼 옆 사람 데우는',
    '丁亥': '작은 등불로 깊은 마음 비추는',
    '丁酉': '촛불 들고 정확한 시간 잡는',
    '丁未': '따뜻한 등불처럼 사람 옆에 오래 머무는',
    '丁巳': '촛불 안에 비밀 한 가지 영리하게 숨긴',
    // 戊(큰 산) ──
    '戊辰': '큰 산처럼 한 번 자리잡으면 흔들리지 않는',
    '戊寅': '산 위에 선 범처럼 시야 넓게 잡는',
    '戊子': '단단한 땅 위 작은 결정 모아 큰일 만드는',
    '戊戌': '큰 산 같은 우직함으로 사람 지키는',
    '戊申': '큰 산 위에서 영리한 길 찾는',
    '戊午': '산 위 말처럼 큰 그림 그리며 내달리는',
    // 己(부드러운 흙) ──
    '己巳': '따뜻한 흙 안에 비밀 한 줌 품은',
    '己卯': '부드러운 흙 위에 살랑살랑 움직이는',
    '己丑': '부드러운 흙처럼 묵묵하게 한 길 가는',
    '己亥': '따뜻한 흙처럼 옆 사람 푹 안아주는',
    '己酉': '부드러운 흙 위에 정확한 시간 감각 가진',
    '己未': '부드러운 흙 위 양처럼 평온한 결 가진',
    // 庚(쇠) ──
    '庚午': '한낮의 말처럼 직진으로 결과 만드는',
    '庚辰': '단단한 검 같은 의지로 큰 일 끝내는',
    '庚寅': '검 든 범처럼 한 발 앞서 움직이는',
    '庚子': '어둠 속 칼처럼 조용히 결정 끝내는',
    '庚戌': '검 든 충견처럼 약속 끝까지 지키는',
    '庚申': '단단한 칼에 영리함 더한',
    // 辛(다듬어진 금속) ── 1995-10-27 男 골든 = 辛卯
    '辛未': '부드러운 결 안에 단단한 칼날 숨긴',
    '辛巳': '차가운 칼 안에 뜨거운 결단 숨긴',
    '辛卯': '다듬어진 칼 안에 부드러운 봄 결 품은',
    '辛丑': '단단한 결 위에 느린 정확함 쌓는',
    '辛亥': '다듬어진 칼 안에 따뜻한 결단 품은',
    '辛酉': '다듬어진 결과 정확한 시간 감각 함께 가진',
    // 壬(큰 물) ──
    '壬申': '큰 강처럼 흘러도 머리 빠르게 돌아가는',
    '壬午': '강물 위 말처럼 거침없이 도전하는',
    '壬辰': '깊은 바다 같은 용 한 마리처럼 큰 그릇 가진',
    '壬寅': '큰 물 위 범처럼 깊고 넓게 움직이는',
    '壬子': '깊은 밤 강물처럼 조용히 흘러가는',
    '壬戌': '깊은 강물 옆 충견처럼 조용한 의리 가진',
    // 癸(이슬) ──
    '癸酉': '이슬처럼 가볍게 다가가서 깊게 남는',
    '癸未': '이슬 머금은 양처럼 부드럽고 영리한',
    '癸巳': '이슬처럼 조용해도 비밀 한 가지 정확히 아는',
    '癸卯': '이슬 머금은 봄처럼 부드럽게 사람을 끄는',
    '癸丑': '이슬 머금은 소처럼 묵묵하게 끝까지 가는',
    '癸亥': '이슬 머금은 따뜻한 마음으로 사람 푹 안는',
  };
