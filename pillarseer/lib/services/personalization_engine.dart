...
class PersonalizationEngine {
  ...
  static PersonalReading buildFor(SajuResult saju, {DateTime? now}) {
    ...
    final seed =
        (chartHash * 1009 + t.year * 366 + t.month * 31 + t.day) & 0x7fffffff;
    ...
    final gAnchorKo = ctx != null
        ? DynamicTextResolver.gyeokgukAnchor(ctx, locale: 'ko')
        : '';
    final gAnchorEn = ctx != null
        ? DynamicTextResolver.gyeokgukAnchor(ctx, locale: 'en')
        : '';
    final yAnchorKo = ctx != null
        ? DynamicTextResolver.yongsinSuffix(ctx, locale: 'ko')
        : '';
    final yAnchorEn = ctx != null
        ? DynamicTextResolver.yongsinSuffix(ctx, locale: 'en')
        : '';
    // Round 79 sprint 6 — 신살 anchor (caution 영역).
    final sAnchorKo = ctx != null
        ? DynamicTextResolver.shinsaAnchor(ctx, locale: 'ko')
        : '';
    final sAnchorEn = ctx != null
        ? DynamicTextResolver.shinsaAnchor(ctx, locale: 'en')
        : '';

    String joinKo(String base, String extra) =>
        (extra.isNotEmpty && !base.contains(extra))
        ? NaturalProseJoiner.append(base, [extra])
        : base;
    String joinEn(String base, String extra) =>
        (extra.isNotEmpty && !base.contains(extra)) ? '$base\n$extra' : base;

    final bodyKoBase = today != null
        ? render(today.koTpl)
        : _fallbackBodyKo(profile);
    final bodyEnBase = today != null
        ? render(today.enTpl)
        : _fallbackBodyEn(profile);
    final actionKoBase = action != null
        ? render(action.koTpl)
        : _fallbackActionKo(profile);
    final actionEnBase = action != null
        ? render(action.enTpl)
        : _fallbackActionEn(profile);
    final cautionKoBase = caution != null
        ? render(caution.koTpl)
        : _fallbackCautionKo(profile);
    final cautionEnBase = caution != null
        ? render(caution.enTpl)
        : _fallbackCautionEn(profile);

    return PersonalReading(
      headlineKo: identity != null
          ? render(identity.koTpl)
          : _fallbackHeadKo(profile),
      headlineEn: identity != null
          ? render(identity.enTpl)
          : _fallbackHeadEn(profile),
  ...
  static String _gnameEn(String g) =>
      {
        '甲': 'a tall-tree type',
        '乙': 'a bending-grass type',
        '丙': 'a sunlight type',
        '丁': 'a candle-warmth type',
        '戊': 'a solid-mountain type',
        '己': 'a fertile-soil type',
        '庚': 'a forged-metal type',
        '辛': 'a jewel-edge type',
        '壬': 'a deep-ocean type',
        '癸': 'a drizzle type',
      }[g] ??
  ...
  static String _elNameEn(String e) =>
      {'木': 'wood', '火': 'fire', '土': 'earth', '金': 'metal', '水': 'water'}[e] ??
      e;
  ...
  static final List<InsightAtom> _atoms = [
    ...
    InsightAtom(
      ...
      condition: (p, t) =>
          (p.season == 'spring' && p.dayMasterElement == '木') ||
    ...
    InsightAtom(
      ...
      priority: 85,
      koTpl:
          '오늘은 새로 시작하기보다 진행 중인 일을 매듭짓는 게 더 잘 풀려요. 한 번에 하나씩, 시작했던 작은 일부터 차근차근 끝내 보세요. 마무리한 하나가 오늘 가장 큰 성취가 돼요.',
      enTpl:
          'Today rewards closing over opening. Go one thing at a time — finish a small task you started earlier. The single closed loop is today\'s biggest win.',
    ...
    InsightAtom(
      ...
      priority: 70,
      koTpl:
          '오늘은 말의 온도를 한 단계 낮추는 게 도움이 돼요. 평소 잘 통하던 표현도 오늘은 살짝 강하게 들릴 수 있으니, 한 박자 늦춰서 부드럽게 전달하세요.',
      enTpl:
          'Cool your tone one notch today. Words that usually land may feel a bit sharp — slow by a beat and soften.',
    ...
    InsightAtom(
      ...
      enTpl:
          'A good day to close a postponed decision. Finishing even one small choice lightens the whole week.',
    ...
    InsightAtom(
      ...
      priority: 70,
      koTpl:
          '오늘 중요한 메시지나 제안은 오전 11시 이후에 보내는 게 더 잘 받아들여져요. 이른 아침보다는 상대가 충분히 깨어 있을 때 닿게 보내세요.',
      enTpl:
          'Send the important message after 11 AM today — better reception when the other side is fully awake.',
    ...
    InsightAtom(
      ...
      priority: 80,
      koTpl:
          '오늘은 {def}이 살짝 부족한 흐름이에요. 큰 결정은 24시간 보류하고, 한 번 자고 일어난 후에 결정해도 늦지 않아요.',
      enTpl:
          'Your {defEn} runs light today. Hold big decisions for 24 hours — sleep on it, then decide.',
      condition: (p, t) => !p.isStrong,
    ),
    InsightAtom(
      topic: 'caution',
      priority: 65,
      koTpl:
          '주말 흐름이에요. 의무감으로 약속을 잡으면 다음 주까지 피로가 따라와요. 정말 보고 싶은 한 사람만 만나도 충분해요.',
      enTpl:
          'Weekend energy. Booking from obligation drags fatigue into next week — meeting one person you really want to see is enough.',
    ...
    InsightAtom(
      ...
      priority: 60,
      koTpl:
          '오늘은 살짝 부족한 {def}을 의식적으로 보태면 흐름이 부드러워져요. 평소 잊기 쉬운 작은 습관 하나가 오늘의 열쇠가 돼요.',
      enTpl:
          'Adding a conscious touch of {defEn} unblocks today. A small habit you usually forget becomes today\'s key.',
      condition: (p, t) => !p.isStrong,
    ),
    // action 추가
    InsightAtom(
      ...
      priority: 60,
      koTpl:
          '오늘 일정 어딘가에 {compKo} 시간을 잠깐이라도 끼워 넣으세요. 길지 않아도 돼요, 의식적으로 한 번이면 충분해요.',
      enTpl:
          'Slot a quick block of {compEn} somewhere in today\'s schedule. It doesn\'t have to be long — one conscious moment is enough.',
      condition: (p, t) => true,
    ),
    // identity 추가 (강한 일간 별)
    InsightAtom(
    ...
    InsightAtom(
      ...
      priority: 75,
      koTpl:
          '오늘은 {dom}이 강한 날이라 새로 시작하는 일보다 정리·마무리에 더 좋은 흐름이에요. 책상 위 한 칸만 정리해도 머리가 가벼워져요.',
      enTpl:
          'A {domEn}-heavy day — better for tidying and closing than for starting. Even clearing one corner of your desk lifts your head.',
      condition: (p, t) => p.dominantEl == '土' || p.dominantEl == '金',
    ),
    InsightAtom(
      topic: 'today',
      priority: 75,
      koTpl:
          '오늘은 {dom}이 풍부한 날이에요. 자기 표현을 늘려도 잘 받아들여지는 흐름이지만, 양을 너무 많이 풀면 듣는 사람이 지칠 수 있으니 핵심만.',
      enTpl:
          '{domEn} energy is abundant today. Expressing more is welcomed, but pouring out too much tires the listener — stay on the core.',
      condition: (p, t) => p.dominantEl == '木' || p.dominantEl == '火',
    ),
    // caution 추가 (요일 + 계절)
    InsightAtom(
      topic: 'caution',
      priority: 70,
      koTpl:
          '오늘은 월요일이에요. 가장 먼저 보내는 한 통의 메시지나 첫 인사의 톤이 한 주 전체 분위기를 정해요. 평소보다 한 단계 더 정성스럽게 시작해 보세요.',
      enTpl:
          'Monday. The first message you send sets the tone of the whole week — start with one extra notch of care.',
    ...
    InsightAtom(
      ...
      priority: 70,
      koTpl:
          '오늘은 금요일이에요. 중요한 결정은 주말을 보내고 월요일에 다시 보세요. 오늘은 결정보다 듣는 시간을 더 길게 가지면 답이 자연스럽게 와요.',
      enTpl:
          'Friday. Hold key decisions until Monday — today, listen longer than you decide, and answers arrive naturally.',
    ...
    InsightAtom(
      ...
      priority: 65,
      koTpl:
          '여름 — {def}을 자연 속에서 흡수하기 좋은 계절이에요. 짧은 산책, 물 한 잔, 잠깐 하늘 올려다보기만 해도 보충돼요.',
      enTpl:
          'Summer — absorb {defEn} from nature. A short walk, a glass of water, looking up at the sky for a minute — that refills it.',
  ...
  static int _chartHash(SajuResult saju) {
    ...
    h =
        h * 31 +
        saju.elements.wood +
        saju.elements.fire * 7 +
        saju.elements.earth * 13 +
        saju.elements.metal * 17 +
