...
//
// 강도 base = 인성 % + 비겁 % (element % 는 이미 지장간·월령·통근 가중 반영).
...
class StrengthService {
  ...
  ///
  /// element 분포 (`wood..water`) 는 이미 지장간·월령·통근 가중이 반영된
  ...
  static ({int score, String label, String labelEn}) judge({
    ...
    const inseong = {'木': '水', '火': '木', '土': '火', '金': '土', '水': '金'};
  ...
  static String guide(String label, {bool ko = false}) {
    if (ko) {
      const koMap = {
        '신강':
            '신강 사주 — 일간 매우 강함. 식상·재성·관성(표현/사업/지위) 운에 가장 빛납니다. 자기 기준을 다듬는 방향 권장.',
        '신왕': '신왕 사주 — 일간 강함. 일을 끝까지 가져가는 힘이 있어요. 외부 흐름을 받아들이는 데 신경.',
        '중화': '중화 사주 — 가장 균형 잡힌 분포. 어떤 흐름이든 잘 받아들여요. 가장 자유로운 사주.',
        '신약': '신약 사주 — 일간 약함. 인성·비겁(배움/동료) 운에 살아납니다. 협력과 휴식이 자원.',
        '신쇠': '신쇠 사주 — 일간 매우 약함. 자기 페이스 보호가 가장 중요. 큰 도전보다 깊이 있는 한 분야 전문.',
      };
      return koMap[label] ?? '';
    }
    const enMap = {
