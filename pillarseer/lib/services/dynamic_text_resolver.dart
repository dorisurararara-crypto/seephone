...
class DynamicTextResolver {
  ...
  static String gyeokgukAnchor(SajuContext ctx, {required String locale}) {
    ...
    if (g.isEmpty) return '';
    if (locale == 'ko') {
      // R86 — 사용자 mandate: 격국 jargon ("정인격" 등) 본문 노출 0.
      // 격국별 변별력은 평어 phrase 차이로 유지 (R78 sprint 7 anchor 변별력 시그니처 보존).
      const map = {
        '정관격': '안정적인 일 흐름이 받쳐줘요.',
        '편관격': '추진력이 한 박자 강해지는 분위기예요.',
        '정인격': '새로 익히는 게 머리에 잘 남는 날이에요.',
        '편인격': '직관이 평소보다 또렷해지는 분위기예요.',
        '정재격': '차곡차곡 쌓이는 흐름이에요.',
        '편재격': '새 거래 신호가 자주 오는 분위기예요.',
        '식신격': '표현이 술술 풀리는 흐름이에요.',
        '상관격': '한 발 빠른 감이 살아나는 분위기예요.',
      };
      return map[g] ?? '';
    } else {
      const map = {
  ...
  static String yongsinSuffix(SajuContext ctx, {required String locale}) {
    ...
    if (locale == 'ko') {
      const map = {
        ...
        '水': '오늘은 잠을 조금만 더 챙겨도 내일 몸이 훨씬 덜 무거워요.',
      };
      return map[y] ?? '';
    } else {
      const map = {
