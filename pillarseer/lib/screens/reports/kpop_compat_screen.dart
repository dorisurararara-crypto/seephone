        ...
        switch (familyIdx) {
          ...
          default:
            ...
            switch (skeletonIdx) {
              ...
              case 4:
                return hasExtra
                    ? '너에게 $shortName${withSubj(shortName)} $relCueKo 자리에 서요 — $blurbExtra.'
                    : '너에게 $shortName${withSubj(shortName)} $relCueKo 자리에 서요 — $blurbPhrase.';
              case 5:
                return stJiSceneKo.isNotEmpty
              ...
              case 6:
                return hasExtra
                    ? '$shortName${withSubj(shortName)} 너에게 보여 주는 면 — $relCueKo 흐름 위의 $blurbExtra.'
                    : '$shortName${withSubj(shortName)} 너에게 보여 주는 면 — $relCueKo 흐름 위의 $blurbPhrase.';
              default:
...
class _KpopAnchors {
  ...
  static const _relPoolKo = {
    ...
    _ElRel.iGenerate: [
      ...
      r'너의 결정이 $shortName의 다음 챕터를 여는 키가 되는 경우가 많아요. 그 권한의 무게를 느낀다면 무리해서 결정하지 말고, "지금은 너에게 맡길게" 라고 넘기는 카드도 자주 써주세요.',
      ...
      r'주는 쪽인 너에게 가끔은 $shortName이 작게라도 되돌려주는 순간이 와요. 그 한 줌의 환원을 가볍게 넘기지 말고 분명히 인정해주면, 상대가 받는 사람 모드에서 벗어나는 데 도움이 돼요.',
    ...
    _ElRel.theyGenerate: [
      ...
      r'$shortName의 자원이 너에게 흘러오는 자리라, 큰 일에 앞서 상대에게 솔직히 상황을 공유하는 게 의외로 가장 큰 효과를 가져요. 비밀 없이 가는 게 이 관계의 전제예요.',
    ...
    _ElRel.iOvercome: [
      ...
      r'$shortName의 행동 패턴이 너에게는 빨리 분석돼요. 그 분석을 평가로 전달하지 말고, 상대가 그 안에서 자기 색을 발견할 수 있도록 질문형 언어로 풀어주세요.',
    ...
    _ElRel.theyOvercome: [
      ...
      r'$shortName이 너에게 자극이 되는 자리라, 가끔은 일부러 그 자극을 피해 거리를 두는 시간도 필요해요. 거리두기가 도망이 아니라 회복이라는 걸 자기한테 분명히 설명해두세요.',
      ...
      r'네가 흔들림 받는 입장이라 갈등 직후 회복 시간이 평균보다 길어요. 그 시간을 자기 약점으로 보지 말고, 너에게 맞는 회복 속도라고 정확히 자기한테 설명해주세요.',
      ...
      r'$shortName의 정확함이 너에게는 자극이자 부담이에요. 그 부담을 줄이려면, "내가 너의 정확함을 100% 다 따라가지는 않아도 돼" 라는 자기 허용이 먼저 필요해요.',
