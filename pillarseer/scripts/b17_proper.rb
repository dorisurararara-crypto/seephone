...
# 1. Build #17 id
code, body = api(:get, "/v1/builds?filter%5Bapp%5D=#{APP_ID}&filter%5Bversion%5D=17&limit=1")
b17_id = JSON.parse(body)['data'].first['id']
puts "Build #17 id: #{b17_id}"

# 2. localizations 조회
code, body = api(:get, "/v1/builds/#{b17_id}/betaBuildLocalizations")
locs = JSON.parse(body)['data']
by_locale = {}
locs.each { |l| by_locale[l['attributes']['locale']] = l['id'] }
puts "locales auto-created: #{by_locale.keys}"

# 3. whatsNew 채우기 (REVIEW SUBMIT 전에!)
ko_text = <<~KO.strip
...
# 4. 외부 그룹 할당
puts "\n외부 그룹 할당..."
code, body = api(:post, "/v1/builds/#{b17_id}/relationships/betaGroups",
                 {data: [{type: 'betaGroups', id: EXTERNAL_GROUP_ID}]})
puts "  HTTP #{code}"

# 5. 잠시 대기 (Apple 가 whatsNew를 internalize 할 시간)
sleep 3
...
code, body = api(:post, '/v1/betaAppReviewSubmissions',
                 {data: {type: 'betaAppReviewSubmissions',
                         relationships: {build: {data: {type: 'builds', id: b17_id}}}}})
puts "  HTTP #{code}: #{body.slice(0, 120)}"
